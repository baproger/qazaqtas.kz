...
Раздел «ИИ-помощник» в меню видят только роли **admin** и **director**
(Gate `use-ai-assistant`). Помощник читает сводку по системе и отвечает
текстом; менять данные он не может.

Переменные в `.env` на сервере:

```
ANTHROPIC_API_KEY=sk-ant-...        # общий ключ (его же использует SEO-генератор)
AI_ASSISTANT_MODEL=claude-opus-5    # можно сменить на claude-sonnet-5, если дорого
AI_ASSISTANT_DAILY_LIMIT=100        # вопросов в сутки на человека; 0 — без лимита
```

Без ключа раздел открывается, но на вопрос отвечает подсказкой, что нужно
добавить `ANTHROPIC_API_KEY` — ошибок в интерфейсе нет.

**Время ответа.** Запрос к модели идёт синхронно и занимает 10–40 секунд.
Если Plesk обрывает такие запросы (502/504), поднимите лимиты для сайта:

```
max_execution_time = 120
```

и в nginx (Дополнительные директивы): `fastcgi_read_timeout 120s;`

Расход: один вопрос ≈ 3–6 тыс. входных и до 1 тыс. выходных токенов.
Сводка по компании кэшируется на 5 минут, поэтому серия вопросов подряд
базу не нагружает.
