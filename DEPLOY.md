...
**Ключ подходит любой из двух:**
- `AIza…` / `AQ.…` — Google Gemini (есть бесплатный тариф), модели
  подбираются автоматически: gemini-3.5-flash-lite → 3.1-flash-lite →
  3.6-flash → 2.5-flash (у каждой своя квота);
- `sk-ant…` — Anthropic (claude-sonnet-5), тариф платный.

Без ключа помощник отвечает бесплатным режимом по данным системы.
