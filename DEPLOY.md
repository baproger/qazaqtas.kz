...
Помощник — агент с инструментами: он не «вспоминает» цифры, а вызывает
инструменты, которые читают базу правами текущего пользователя (сводный
отчёт, сделки, просрочка, склад, цех, задачи, сотрудник, клиент). Цифры
финансового отчёта считаются тем же кодом, что и страница «Сводный отчёт».

**Куда вставить ключ:** ERP → Настройки → раздел «ИИ-помощник» → поле
«Ключ Google Gemini». Поле видит только администратор. Бесплатный ключ
берётся на aistudio.google.com/apikey (кнопка «Create API key»), выглядит
как `AIza…`. Ключ шифруется в базе и в браузер не отдаётся.

Модели Gemini подбираются сами: gemini-3.5-flash-lite → 3.1-flash-lite →
3.6-flash → 2.5-flash (у каждой своя квота). Ключ `sk-ant…` тоже принимается —
тогда помощник работает через Anthropic (платно).

Без ключа помощник отвечает бесплатным режимом по данным системы, и такие
ответы лимитом не ограничиваются — считается только обращение к внешнему ИИ
(8 вопросов в минуту).

Переменная окружения `AI_API_KEY` осталась запасным путём, если ключ удобнее
держать на сервере.
...
**Ключ проще ввести в системе:** Настройки → ИИ-помощник → «Ключ Anthropic».
Поле видит только администратор, ключ хранится в базе зашифрованным (APP_KEY)
и в браузер не отдаётся — показываются лишь последние 4 символа. Тот же ключ
используется и генератором SEO. Вариант через `.env` остаётся запасным:
...
**Ключ нужен не для всего.** Без `ANTHROPIC_API_KEY` помощник работает в
режиме «без ИИ» и отвечает готовыми выборками прямо из базы — бесплатно и
мгновенно: просроченные сделки, остатки склада, заказы в цехе, задачи по
людям, поступления денег, воронка сделок, топ клиентов, рейтинг менеджеров,
ходовые товары, общая сводка. К любому вопросу можно добавить рамки —
период («за месяц», «сегодня»), имя сотрудника или город. Ключ добавляет
свободные формулировки и деловые тексты. Если ИИ временно недоступен
(перегрузка, сеть), типовые вопросы тоже обслуживаются из базы.
