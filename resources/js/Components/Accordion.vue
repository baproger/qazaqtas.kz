...
<template>
    <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-card dark:border-slate-800 dark:bg-slate-900">
        <button type="button" class="flex w-full items-center gap-2 px-5 py-3 text-left transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-slate-800/60"
            :aria-expanded="isOpen" @click="isOpen = !isOpen">
            <slot name="icon" />
            <span class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ title }}</span>
            <span v-if="count !== null" class="rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400">{{ count }}</span>
            <slot name="badge" />
        ...
        <div v-show="isOpen" class="border-t border-slate-100 dark:border-slate-800 px-5 py-4">
            <slot />
        </div>
    </div>
</template>
