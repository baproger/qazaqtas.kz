...
const remove = async (c) => { if (await confirmDialog({ title: tr('Удалить комментарий'), message: tr('Комментарий будет удалён.'), confirmText: tr('Удалить'), danger: true })) router.delete(route('comments.destroy', c.id), { preserveScroll: true }); };
