...
<template>
    <div ref="root" class="relative">
        <button type="button" @click="open = !open"
            class="flex min-h-[2.5rem] w-full items-center gap-1.5 rounded-xl border border-white/60 bg-white/70 px-3 py-1.5 text-left text-sm shadow-soft backdrop-blur transition hover:bg-white/90 focus:outline-none focus:ring-2 focus:ring-indigo-400"
        ...
        <transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0 -translate-y-1 scale-[0.98]"
            ...
            <div v-if="open" class="absolute left-0 right-0 z-30 mt-1.5 overflow-hidden rounded-xl border border-white/60 bg-white/80 shadow-soft-lg backdrop-blur-xl">
                <div v-if="options.length > 6" class="border-b border-slate-100/80 p-2">
                ...
                <ul class="max-h-56 overflow-y-auto p-1">
                    <li v-for="o in filtered" :key="o.value">
                        <button type="button" @click="toggle(o.value)"
                            class="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-1.5 text-left text-sm transition hover:bg-indigo-50/80">
                            <span class="flex h-4 w-4 shrink-0 items-center justify-center rounded border transition"
                                :class="modelValue.includes(o.value) ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-300 bg-white'">
                            ...
                            <span class="truncate" :class="modelValue.includes(o.value) ? 'font-medium text-slate-900' : 'text-slate-700'">{{ o.label }}</span>
                        </button>
                    </li>
                    <li v-if="!filtered.length" class="px-2.5 py-2 text-xs text-slate-400">—</li>
                </ul>
                <div v-if="modelValue.length" class="border-t border-slate-100/80 px-2 py-1.5 text-right">
                    <button type="button" @click="clear" class="text-xs font-medium text-slate-500 hover:text-rose-600">{{ emptyLabel ? '× ' + emptyLabel : '×' }}</button>
                </div>
            </div>
        </transition>
    </div>
