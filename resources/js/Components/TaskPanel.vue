...
const remove = async (t) => { if (await confirmDialog({ title: tr('Удалить задачу'), message: tr('Задача будет удалена.'), confirmText: tr('Удалить'), danger: true })) router.delete(route('tasks.destroy', t.id), { preserveScroll: true }); };
...
<template>
    <div class="space-y-2">
        ...
        <div v-if="adding" class="rounded-xl border border-dashed border-slate-300 p-4">
            ...
            <div class="mb-2 grid grid-cols-2 gap-2">
                <select v-model="form.assignee_id" class="rounded-lg border-slate-300 text-sm transition duration-150 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20">
                    <option value="">{{ $e('Исполнитель…') }}</option>
                ...
                <select v-model="form.priority" class="rounded-lg border-slate-300 text-sm transition duration-150 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20">
                    <option value="low">{{ $e('Низкий') }}</option><option value="medium">{{ $e('Средний') }}</option><option value="high">{{ $e('Высокий') }}</option>
                </select>
            </div>
        ...
        <Modal :show="!!editing" @close="editing = null">
            <div class="p-6">
                <h2 class="mb-4 text-lg font-semibold">{{ $e('Редактировать задачу') }}</h2>
                <div class="space-y-3">
                    <div><InputLabel :value="$e('Название')" /><TextInput v-model="editForm.title" class="mt-1 w-full" /></div>
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            <InputLabel :value="$e('Исполнитель')" />
                        ...
                        <div>
                            ...
                            <select v-model="editForm.status" class="mt-1 w-full rounded-lg border-slate-300 text-sm transition duration-150 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20">
                                <option value="new">{{ $e('Новая') }}</option><option value="in_progress">{{ $e('В работе') }}</option>
                                <option value="review">{{ $e('Проверка') }}</option><option value="done">{{ $e('Готово') }}</option>
                            </select>
                        </div>
                    </div>
                    <div><InputLabel :value="$e('Срок (дата и время)')" /><TextInput v-model="editForm.due_date" type="datetime-local" class="mt-1 w-full" /></div>
                    <div><InputLabel :value="$e('Заметка')" /><textarea v-model="editForm.note" rows="2" class="mt-1 w-full rounded-lg border-slate-300 text-sm transition duration-150 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20"></textarea></div>
                    <div><InputLabel :value="$e('Описание')" /><textarea v-model="editForm.description" rows="2" class="mt-1 w-full rounded-lg border-slate-300 text-sm transition duration-150 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20"></textarea></div>
                </div>
