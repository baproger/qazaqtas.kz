    ...
    <div class="space-y-0">
        <div v-for="(log, i) in history" :key="log.id" class="flex gap-3">
            <div class="flex flex-col items-center">
                ...
                <span v-if="i < history.length - 1" class="w-px flex-1 bg-slate-200 dark:bg-slate-800"></span>
            </div>
            <div class="pb-4 text-sm">
                <div class="text-slate-800 dark:text-slate-200">
                    <span class="font-medium">{{ log.user?.name ?? $e('Система') }}</span>
                    <span class="text-slate-500 dark:text-slate-400"> {{ (actionLabel[log.action] || log.action).toLowerCase() }}</span>
                    <span v-if="log.field_name" class="text-slate-500 dark:text-slate-400"> «{{ fieldRu[log.field_name] || log.field_name }}»</span>
                </div>
                <div v-if="log.field_name" class="mt-0.5 text-xs">
                    <span class="text-rose-500 line-through dark:text-rose-400">{{ log.old_value ?? '∅' }}</span>
                    <span class="mx-1">→</span>
                    <span class="text-emerald-600 dark:text-emerald-400">{{ log.new_value ?? '∅' }}</span>
                </div>
        ...
        <div v-if="!history.length" class="flex flex-col items-center gap-2 py-8 text-center">
            <svg class="h-10 w-10 text-slate-300 dark:text-slate-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>
