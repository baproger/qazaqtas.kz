    ...
    <div>
        <div class="relative flex items-stretch gap-2">
            ...
            <div class="min-w-0 flex-1 rounded-xl border bg-white shadow-sm transition-colors duration-150"
                :class="isSelected ? 'border-bx-400 ring-1 ring-bx-200' : 'border-slate-200/60 hover:border-bx-200'">
                <div class="flex flex-wrap items-center gap-x-2.5 gap-y-1.5 px-3 py-2">
                    ...
                    <button type="button" @click="ctx.pick(node)" class="min-w-0 flex-1 text-left">
                        <div class="truncate text-[12px] font-semibold leading-tight text-slate-900">{{ node.name }}</div>
                        <div v-if="node.head" class="mt-0.5 flex items-center gap-1.5">
                            <Avatar :name="node.head.name" :src="node.head.avatar" :size="16" />
                            <span class="truncate text-[10px] text-slate-500">{{ node.head.name }}</span>
                        </div>
                        <div v-else class="mt-0.5 text-[10px] text-amber-600">{{ $e('руководитель не назначен') }}</div>
                    </button>
                    ...
                    <button v-if="node.people.length" type="button" @click="ctx.canManage && ctx.addPerson(node)"
                        ...
                        <span v-if="node.people.length > 4" class="ml-1.5 text-[10px] font-semibold text-slate-500">+{{ node.people.length - 4 }}</span>
                    </button>

                    <span class="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] font-semibold tabular-nums text-slate-500"
                    ...
                    <template v-if="ctx.canManage">
                        ...
                        <button type="button" @click="ctx.openCreate(node.id)" :title="$e('Добавить подотдел')"
                            class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full border border-slate-200 text-slate-400 transition-colors duration-150 hover:border-bx-400 hover:text-bx-600">
