    ...
    <div class="space-y-6">
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-3 gap-3">
                <div class="rounded-xl bg-indigo-50 p-4 dark:bg-indigo-500/10"><div class="text-xs font-medium text-indigo-700 dark:text-indigo-300">{{ $e('Сумма договора') }}</div><div class="mt-0.5 text-lg font-bold tabular-nums text-indigo-700 dark:text-indigo-300">{{ money(finance.budget) }}</div></div>
                <div class="rounded-xl bg-emerald-50 p-4 dark:bg-emerald-500/10"><div class="text-xs font-medium text-emerald-700 dark:text-emerald-400">{{ $e('Аванс (оплачено)') }}</div><div class="mt-0.5 text-lg font-bold tabular-nums text-emerald-700 dark:text-emerald-400">{{ money(finance.income) }}</div></div>
                <div class="rounded-xl bg-rose-50 p-4 dark:bg-rose-500/10"><div class="text-xs font-medium text-rose-700 dark:text-rose-400">{{ $e('Расходы') }}</div><div class="mt-0.5 text-lg font-bold tabular-nums text-rose-700 dark:text-rose-400">{{ money(finance.expense) }}</div></div>
            </div>

            <!-- income vs expense bar -->
            <div>
                <div class="mb-1.5 flex justify-between text-xs font-medium text-slate-500 dark:text-slate-400"><span>{{ $e('Доход vs Расходы') }}</span><span>{{ $e('наценка') }} {{ finance.markup }}%</span></div>
                <div class="flex h-3 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800/60">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="rounded-xl border border-slate-200 p-4 dark:border-slate-800/80">
                    <div class="text-xs font-semibold uppercase tracking-wide text-slate-400">{{ $e('Факт (по оплатам)') }}</div>
                    <div class="mt-1 flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">{{ $e('Прибыль') }}</span><span class="font-bold tabular-nums" :class="finance.profit >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'">{{ money(finance.profit) }}</span></div>
                    <div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">{{ $e('Маржа') }}</span><span class="font-bold tabular-nums">{{ finance.margin }}%</span></div>
                    <div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">{{ $e('Доля расходов') }}</span><span class="tabular-nums">{{ finance.expenseRatio }}%</span></div>
                </div>
                <div class="rounded-xl border border-slate-200 p-4 dark:border-slate-800/80">
                    <div class="text-xs font-semibold uppercase tracking-wide text-slate-400">{{ $e('План (по сумме сделки)') }}</div>
                    <div class="mt-1 flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">{{ $e('Выгода') }}</span><span class="font-bold tabular-nums" :class="finance.plannedProfit >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'">{{ money(finance.plannedProfit) }}</span></div>
                    <div class="flex justify-between text-sm"><span class="text-slate-500 dark:text-slate-400">{{ $e('Маржа') }}</span><span class="font-bold tabular-nums">{{ finance.plannedMargin }}%</span></div>
                    <div v-if="finance.plannedProfit < 0" class="mt-1 flex items-center gap-1 text-xs font-medium text-rose-600 dark:text-rose-400">
        ...
        <div>
            <div class="mb-3 flex items-center justify-between">
                <h4 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $e('Аванс') }}</h4>
            ...
            <div v-if="showInvoice" class="mb-3 rounded-xl border border-dashed border-slate-300 p-4 dark:border-slate-600">
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        ...
                        <div v-if="Number(invoiceForm.amount) <= 0" class="mt-1 text-xs font-medium text-red-600 dark:text-rose-400">{{ $e('Введите сумму счёта') }}</div>
                    </div>
                ...
                <div v-if="invoiceForm.errors.amount" class="mt-1 text-xs text-red-600 dark:text-rose-400">{{ invoiceForm.errors.amount }}</div>
            ...
            <div class="space-y-2">
                <div v-for="inv in invoices" :key="inv.id" class="rounded-xl bg-slate-50 dark:bg-slate-800/50 p-4 text-sm">
                    <div class="flex items-center justify-between">
                        <div>
                            <span class="font-medium text-slate-900 dark:text-slate-100">{{ inv.number }}</span>
                            <span class="ml-2 tabular-nums text-slate-500 dark:text-slate-400">{{ money(inv.amount) }}</span>
                            <span class="ml-2 rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400">{{ $e('оплачено') }} {{ money(inv.payments_sum_amount ?? 0) }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <StatusBadge :status="inv.status" />
                            <button class="text-xs font-semibold text-indigo-600 transition-colors duration-150 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300" @click="openPay(inv)">{{ $e('Оплата') }}</button>
                            <button class="rounded p-1 text-slate-400 transition-colors duration-150 hover:text-rose-600 dark:hover:text-rose-400" :title="$e('Удалить')" @click="delInvoice(inv)">
                    ...
                    <div v-if="payFor === inv.id" class="mt-2 flex items-end gap-2 border-t border-slate-200 pt-2 dark:border-slate-800/80">
                        ...
                        <button class="text-sm text-slate-500 transition-colors duration-150 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300" @click="payFor = null">{{ $e('Отмена') }}</button>
                    </div>
                </div>
                <div v-if="!invoices.length" class="flex flex-col items-center gap-3 py-8 text-center">
                    <svg class="h-10 w-10 text-slate-300 dark:text-slate-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6M9 13h6M9 17h6"/></svg>
        ...
        <div>
            <div class="mb-3 flex items-center justify-between">
                <h4 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $e('Расходы') }}</h4>
            ...
            <div v-if="showExpense" class="mb-3 rounded-xl border border-dashed border-slate-300 p-4 dark:border-slate-600">
                ...
                <div class="mb-3 flex flex-wrap gap-2">
                    <button v-for="m in [
                        ...
                        :class="expenseMode === m.k ? 'border-indigo-500 bg-indigo-50 text-indigo-700 ring-1 ring-indigo-500 dark:bg-indigo-500/10 dark:text-indigo-300' : 'border-slate-200 text-slate-500 hover:border-slate-300 dark:border-slate-800/80 dark:text-slate-400 dark:hover:border-slate-600'">
                        {{ m.l }}
                    </button>
                    <button v-if="materials.length" type="button" @click="expenseMode = 'material'"
                        class="rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all"
                        :class="expenseMode === 'material' ? 'border-indigo-500 bg-indigo-50 text-indigo-700 ring-1 ring-indigo-500 dark:bg-indigo-500/10 dark:text-indigo-300' : 'border-slate-200 text-slate-500 hover:border-slate-300 dark:border-slate-800/80 dark:text-slate-400 dark:hover:border-slate-600'">
                ...
                <div v-if="expenseMode === 'material' && materials.length" class="mb-2 grid grid-cols-2 gap-2">
                    ...
                    <div v-if="selectedMaterial" class="flex items-center text-xs"
                        :class="Number(expenseForm.qty) > Number(selectedMaterial.quantity) ? 'text-rose-600 font-semibold dark:text-rose-400' : 'text-slate-500 dark:text-slate-400'">
                    ...
                    <div v-if="selectedMaterial && materialPrice > 0" class="col-span-2 rounded-lg bg-indigo-50 px-3 py-2 text-sm text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300">
                ...
                <div class="mt-2 flex gap-2">
                    <button type="button" @click="expenseForm.payment_method = 'cash'"
                        class="rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all"
                        :class="expenseForm.payment_method === 'cash' ? 'border-emerald-500 bg-emerald-100 text-emerald-700 ring-1 ring-emerald-500 dark:bg-emerald-500/20 dark:text-emerald-400' : 'border-slate-200 bg-white text-slate-500 hover:border-slate-300 dark:border-slate-800/80 dark:bg-slate-800 dark:text-slate-400 dark:hover:border-slate-600'">{{ $e('Наличные') }}</button>
                    <button type="button" @click="expenseForm.payment_method = 'bank'"
                        class="rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all"
                        :class="expenseForm.payment_method === 'bank' ? 'border-sky-500 bg-sky-100 text-sky-700 ring-1 ring-sky-500 dark:bg-sky-500/20 dark:text-sky-400' : 'border-slate-200 bg-white text-slate-500 hover:border-slate-300 dark:border-slate-800/80 dark:bg-slate-800 dark:text-slate-400 dark:hover:border-slate-600'">{{ $e('Банк (счёт)') }}</button>
                </div>
                <!-- Остатки из поступлений (видит бухгалтер): расход списывается отсюда -->
                <div v-if="balances" class="mt-1.5 text-xs" :class="overBalance ? 'font-semibold text-rose-600 dark:text-rose-400' : 'text-slate-400'">
                ...
                <div v-if="expenseMode === 'other'" class="mt-2">
                    <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $e('Чек (фото или PDF) — можно прикрепить сейчас, иначе бухгалтер добавит при подтверждении') }}</label>
                    ...
                    <div v-if="expenseForm.errors.file" class="mt-1 text-xs text-red-600 dark:text-rose-400">{{ expenseForm.errors.file }}</div>
                </div>
            ...
            <div class="space-y-2">
                <div v-for="e in expenses" :key="e.id" class="rounded-xl bg-slate-50 dark:bg-slate-800/50 p-4 text-sm">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <div>
                                <span class="font-medium tabular-nums text-slate-900 dark:text-slate-100">{{ money(e.amount) }}</span>
                                <span v-if="expenseTypeLabels[e.type]" class="ml-2 rounded bg-sky-50 px-1.5 py-0.5 text-xs font-semibold text-sky-700 ring-1 ring-sky-200 dark:bg-sky-500/10 dark:text-sky-400">{{ expenseTypeLabels[e.type] }}</span>
                                <span class="ml-2 text-slate-500 dark:text-slate-400">{{ e.description }}</span>
                            </div>
                            <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-slate-400">
                                <span v-if="e.material" class="rounded-full bg-indigo-100 px-2 py-0.5 font-medium text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300">{{ $e('склад:') }} {{ e.material.name }} × {{ qtyNum(e.qty) }} {{ e.material.unit }}</span>
                                <span v-if="e.payment_method" class="rounded-full bg-slate-200 px-2 py-0.5 font-medium text-slate-600 dark:bg-slate-700 dark:text-slate-300">{{ e.payment_method === 'cash' ? $e('наличные') : $e('банк') }}</span>
                                ...
                                <button v-if="e.file_path" type="button" @click="openReceipt(e)"
                                    class="inline-flex items-center gap-1 font-medium text-indigo-600 transition-colors duration-150 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300">
                                ...
                                <span v-else-if="!e.material" class="rounded-full bg-amber-100 px-2 py-0.5 font-medium text-amber-700 dark:bg-amber-500/20 dark:text-amber-400">{{ $e('без чека') }}</span>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <span v-if="e.status === 'confirmed'" class="rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-semibold text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400">{{ $e('Подтверждён') }}</span>
                            <span v-else-if="e.status === 'pending'" class="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-700 dark:bg-amber-500/20 dark:text-amber-400">{{ $e('Ждёт бухгалтера') }}</span>
                            <span v-else class="rounded-full bg-slate-200 px-2.5 py-0.5 text-xs font-semibold text-slate-600 dark:bg-slate-700 dark:text-slate-300">{{ $e('Черновик') }}</span>
                            ...
                            <button v-if="canConfirm" class="rounded p-1 text-slate-400 transition-colors duration-150 hover:text-rose-600 dark:hover:text-rose-400" :title="$e('Удалить')" @click="delExpense(e)">
                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6M10 11v6M14 11v6"/></svg>
                            </button>
                        </div>
                    </div>
                    <!-- Подтверждение бухгалтером: способ оплаты (касса) + чек -->
                    <div v-if="confirmFor === e.id" class="mt-3 rounded-lg border border-dashed border-emerald-300 bg-emerald-50/50 p-3 dark:border-emerald-500/40 dark:bg-emerald-500/10">
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="text-xs font-medium text-slate-500 dark:text-slate-400">{{ $e('Оплата:') }}</span>
                            <button type="button" @click="confirmForm.payment_method = 'cash'"
                                class="rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all"
                                :class="confirmForm.payment_method === 'cash' ? 'border-emerald-500 bg-emerald-100 text-emerald-700 ring-1 ring-emerald-500 dark:bg-emerald-500/20 dark:text-emerald-400' : 'border-slate-200 bg-white text-slate-500 dark:border-slate-800/80 dark:bg-slate-800 dark:text-slate-400'">{{ $e('Наличные') }}</button>
                            <button type="button" @click="confirmForm.payment_method = 'bank'"
                                class="rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all"
                                :class="confirmForm.payment_method === 'bank' ? 'border-emerald-500 bg-emerald-100 text-emerald-700 ring-1 ring-emerald-500 dark:bg-emerald-500/20 dark:text-emerald-400' : 'border-slate-200 bg-white text-slate-500 dark:border-slate-800/80 dark:bg-slate-800 dark:text-slate-400'">{{ $e('Банк (счёт)') }}</button>
                        </div>
                        <div v-if="!e.file_path" class="mt-2">
                            <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $e('Чек (обязателен для подтверждения)') }}</label>
                        ...
                        <div v-if="confirmForm.errors.file" class="mt-1 text-xs text-red-600 dark:text-rose-400">{{ confirmForm.errors.file }}</div>
                        <div v-if="confirmForm.errors.payment_method" class="mt-1 text-xs text-red-600 dark:text-rose-400">{{ confirmForm.errors.payment_method }}</div>
                        <div class="mt-2 flex gap-2">
                            ...
                            <button class="rounded-lg bg-white px-4 py-1.5 text-xs font-medium text-slate-500 ring-1 ring-slate-200 transition-colors duration-150 hover:bg-slate-100 dark:bg-slate-800 dark:text-slate-400 dark:ring-slate-800 dark:hover:bg-slate-700" @click="confirmFor = null">{{ $e('Отмена') }}</button>
                        </div>
                    </div>
                </div>
                <div v-if="!expenses.length" class="flex flex-col items-center gap-3 py-8 text-center">
                    <svg class="h-10 w-10 text-slate-300 dark:text-slate-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="2.5"/><path d="M6 12h.01M18 12h.01"/></svg>
        ...
        <Modal :show="!!receiptView" max-width="2xl" @close="closeReceipt">
            <div class="bg-white dark:bg-slate-900">
                <div class="flex items-center justify-between border-b px-4 py-2">
                    <h4 class="font-semibold text-slate-700 dark:text-slate-300">{{ $e('Чек') }}</h4>
                    <div class="flex items-center gap-3">
                        <a :href="receiptView?.url" target="_blank" class="text-sm font-medium text-indigo-600 transition-colors duration-150 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300">{{ $e('Открыть в новой вкладке') }}</a>
                        <button type="button" class="rounded p-1 text-slate-400 transition-colors duration-150 hover:text-slate-600 dark:hover:text-slate-300" @click="closeReceipt">
