...
const remove = async (row) => {
    if (await confirmDialog({ title: tr('Удалить строку ДДС'), message: `«${row.name}» будет удалена из сводки.`, confirmText: tr('Удалить'), danger: true })) {
            ...
            <div class="xl:col-span-2 overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead class="text-left text-xs uppercase text-slate-400">
                        <tr>
                            <th class="py-2 pr-3">{{ $e('Компания') }}</th><th class="py-2 pr-3">{{ $e('Банк') }}</th>
                            <th class="py-2 pr-3 text-right">{{ $e('Фактический остаток') }}</th>
                            <th class="py-2 pr-3 text-right">{{ $e('Дебиторский') }}</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr v-if="!dds.accounts.length && addingKind !== 'account'">
                            <td colspan="5" class="py-5 text-center text-xs text-slate-400">{{ $e('Строк пока нет — добавьте компании и остатки') }}</td>
                        </tr>
                    </tbody>
                    <tfoot>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr v-if="!dds.debts.length && addingKind !== 'debt'">
                            <td colspan="3" class="py-5 text-center text-xs text-slate-400">{{ $e('Долгов нет') }}</td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr class="border-t-2 border-slate-200 font-semibold">
