...
<template>
    ...
    <AppLayout>
        ...
        <div class="flex rounded-2xl bg-white/70 p-1 text-sm shadow-soft backdrop-blur w-fit dark:bg-slate-900/70">
            <button v-for="(l, k) in statuses" :key="k" type="button" @click="setStatus(k)"
                class="flex items-center gap-1.5 rounded-xl px-4 py-1.5 font-medium transition" :class="status === k ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' : 'text-slate-600 hover:bg-white dark:text-slate-300 dark:hover:bg-slate-800'">
                {{ l }} <span class="rounded-full px-1.5 text-xs" :class="status === k ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800/60'">{{ counts[k] ?? 0 }}</span>
            </button>
        </div>
        <button type="button" @click="catsOpen = true" class="rounded-xl border border-indigo-300/60 bg-indigo-500/15 px-4 py-2 text-sm font-semibold text-indigo-700 backdrop-blur transition hover:bg-indigo-500/25 dark:text-indigo-300">🗂 {{ $e('Категории') }} <span class="opacity-60">{{ categories.length }}</span></button>
        </div>

        <div class="grid gap-3">
            <div v-for="s in services.data" :key="s.id" class="flex flex-wrap items-start gap-4 rounded-2xl border border-white/60 bg-white/75 p-4 shadow-soft backdrop-blur-md dark:border-slate-800/80 dark:bg-slate-900/70">
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500 dark:bg-slate-800/60 dark:text-slate-400">{{ s.category?.name }}</span>
                        <span v-if="s.price" class="whitespace-nowrap rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300">{{ new Intl.NumberFormat('ru-RU').format(s.price) }} ₸</span>
                        <span class="text-xs text-slate-400">{{ fmt(s.created_at) }}</span>
                    </div>
                    <p class="mt-1 whitespace-pre-line text-sm text-slate-600 dark:text-slate-300">{{ s.description_full }}</p>
                    <div class="mt-1.5 text-xs text-slate-500 dark:text-slate-400">👤 {{ s.partner?.name }} · {{ s.contact_name }} · {{ s.contact_phone }}<template v-if="s.city"> · {{ s.city }}</template></div>
                    <div v-if="s.rejection_reason" class="mt-1 rounded-lg bg-rose-50 px-2 py-1 text-xs text-rose-700 dark:bg-rose-500/10 dark:text-rose-400">{{ $e('Причина:') }} {{ s.rejection_reason }}</div>
                </div>
                <div v-if="s.status === 'pending'" class="flex shrink-0 flex-col gap-2">
                    <button @click="approve(s)" class="rounded-xl border border-emerald-300/60 bg-emerald-500/15 px-4 py-2 text-sm font-semibold text-emerald-700 backdrop-blur transition hover:bg-emerald-500/25 dark:text-emerald-400">✓ {{ $e('Одобрить') }}</button>
                    <button @click="rejecting = s" class="rounded-xl border border-rose-300/60 bg-rose-500/10 px-4 py-2 text-sm font-semibold text-rose-700 backdrop-blur transition hover:bg-rose-500/20 dark:text-rose-400">✕ {{ $e('Отклонить') }}</button>
                </div>
            </div>
            <div v-if="!services.data.length" class="rounded-3xl border border-dashed border-slate-200 bg-white/60 p-12 text-center text-sm text-slate-400 backdrop-blur dark:border-slate-800/80 dark:bg-slate-900/70">{{ $e('Заявок нет') }}</div>
        </div>
        ...
        <div v-if="catsOpen" class="fixed inset-0 z-40 flex items-start justify-center overflow-y-auto bg-slate-900/30 p-4 backdrop-blur-sm sm:p-10" @click.self="catsOpen = false">
            <div class="w-full max-w-lg rounded-3xl border border-white/60 bg-white/95 p-6 shadow-soft-lg backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/70">
                <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">{{ $e('Категории услуг') }}</h2>
                <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{{ $e('Их видят партнёры в форме и посетители в каталоге. Удаление не трогает услуги — они остаются «без категории».') }}</p>
                ...
                <div class="mt-3 divide-y divide-slate-100 dark:divide-slate-800">
                    <div v-for="c in categories" :key="c.id" class="flex items-center gap-2 py-2" :class="c.is_active ? '' : 'opacity-50'">
                        ...
                        <button type="button" @click="toggleCat(c)" class="rounded-lg px-2 py-1 text-xs font-medium" :class="c.is_active ? 'text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800/60' : 'text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-500/10'">{{ c.is_active ? $e('Скрыть') : $e('Показать') }}</button>
                        <button type="button" @click="delCat(c)" class="rounded-lg px-2 py-1 text-xs font-medium text-slate-400 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-500/10 dark:hover:text-rose-400">×</button>
                    </div>
                </div>
        ...
        <div v-if="rejecting" class="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/30 p-4 backdrop-blur-sm" @click.self="rejecting = null">
            <div class="w-full max-w-md rounded-3xl border border-white/60 bg-white/95 p-6 shadow-soft-lg backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/70">
                <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">{{ $e('Отклонить услугу') }}</h2>
                <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">«{{ rejecting.title }}» — {{ $e('партнёр увидит причину и сможет исправить.') }}</p>
