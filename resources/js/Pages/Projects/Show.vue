    ...
    <AppLayout>
        <template #header>
            <div class="flex min-w-0 items-center gap-3">
                ...
                <span class="flex-shrink-0 whitespace-nowrap rounded-md bg-slate-100 dark:bg-slate-800/60 px-2 py-0.5 text-xs font-semibold text-slate-500 dark:text-slate-400">{{ project.number }}</span>
            </div>
        </template>

        <!-- Process bar: the main thing workshop staff need -->
        <div class="mb-4 rounded-2xl bg-white dark:bg-slate-900/70 p-5 border border-slate-100 dark:border-slate-800 shadow-sm">
            <div class="flex items-center gap-2 overflow-x-auto pb-1">
                <template v-for="(stage, i) in stages" :key="stage.id">
                    <button @click="moveStage(stage.id)"
                        :class="stage.id === project.project_stage_id ? 'text-white shadow' : 'bg-slate-100 dark:bg-slate-800/60 text-slate-600 dark:text-slate-300 hover:bg-slate-200'"
                    ...
                    <span v-if="i < stages.length - 1" class="text-slate-300 dark:text-slate-600">›</span>
                </template>
            </div>
            <div class="mt-4 border-t pt-4">
                ...
                <div v-else-if="project.status !== 'completed'" class="flex flex-wrap items-center gap-3">
                    ...
                    <span v-if="unfinishedItems > 0" class="text-sm text-amber-700 dark:text-amber-400">
                        {{ $e('Не закончено товаров:') }} <b>{{ unfinishedItems }}</b> — {{ $e('отметьте их ниже') }}
                    </span>
                </div>
                <span v-else class="inline-flex items-center gap-2 text-sm font-semibold text-green-600 dark:text-green-400">{{ $e('✓ Отправлено на «Логистику»') }}</span>
            </div>
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div class="lg:col-span-2 space-y-4">
                <!-- Что делать и как это выглядит — первым экраном и рядом:
                     ...
                     не присылает. -->
                <div class="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-5 shadow-sm">
                    <div class="mb-3 flex items-baseline gap-2">
                        <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $e('Что делать') }}</h3>
                ...
                <div class="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-5 shadow-sm">
                    <div class="mb-3 flex items-baseline gap-2">
                        <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $e('Общие фото заказа') }}</h3>
                        <span v-if="photos.length" class="rounded-full bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 text-xs font-semibold text-indigo-600 dark:text-indigo-400">{{ photos.length }}</span>
                    </div>
                    <PhotoPanel :documents="attachments" entity-type="project" :entity-id="project.id" />
                </div>

                <div class="rounded-2xl bg-white dark:bg-slate-900/70 p-5 border border-slate-100 dark:border-slate-800 shadow-sm">
                    <div class="mb-4 flex flex-wrap gap-4 border-b text-sm">
                        <button :class="tab==='info' ? 'border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'" class="pb-2 transition-colors" @click="tab='info'">{{ $e('Информация') }}</button>
                        <button :class="tab==='tasks' ? 'border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'" class="pb-2 transition-colors" @click="tab='tasks'">{{ $e('Задачи (') }}{{ project.tasks.length }})</button>
                        <button v-if="canSeeMoney" :class="tab==='finance' ? 'border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'" class="pb-2 transition-colors" @click="tab='finance'">{{ $e('Финансы') }}</button>
                        <button :class="tab==='docs' ? 'border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'" class="pb-2 transition-colors" @click="tab='docs'">{{ $e('Документы (') }}{{ files.length }})</button>
                        <button :class="tab==='comments' ? 'border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'" class="pb-2 transition-colors" @click="tab='comments'">{{ $e('Комментарии (') }}{{ project.comments.length }})</button>
                        <button :class="tab==='history' ? 'border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'" class="pb-2 transition-colors" @click="tab='history'">{{ $e('История') }}</button>
                    </div>
                    ...
                    <div v-if="tab==='info'" class="text-sm">
                        ...
                        <div class="grid grid-cols-2 gap-x-5 gap-y-3 sm:grid-cols-3">
                            <div>
                                ...
                                <div class="mt-0.5 font-semibold text-slate-900 dark:text-slate-100">{{ project.client?.name ?? deal?.company_name ?? '—' }}</div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-0.5 font-medium text-slate-900 dark:text-slate-100">{{ deal?.responsible?.name ?? project.responsible?.name ?? '—' }}</div>
                            </div>
                            <div>
                                <div class="text-xs uppercase tracking-wide text-slate-400">{{ $e('Бригадир') }}</div>
                                <div class="mt-0.5 font-medium text-slate-900 dark:text-slate-100">{{ deal?.foreman?.name ?? '—' }}</div>
                            </div>
                            <div class="col-span-2">
                                <div class="text-xs uppercase tracking-wide text-slate-400">{{ $e('Адрес') }}</div>
                                <div class="mt-0.5 font-medium text-slate-900 dark:text-slate-100">📍 {{ deal?.address || '—' }}</div>
                            </div>
                            <div>
                                <div class="text-xs uppercase tracking-wide text-slate-400">{{ $e('Срок') }}</div>
                                <div class="mt-0.5 font-medium text-slate-900 dark:text-slate-100">{{ formatDate(project.deadline ?? deal?.deadline) }}</div>
                            </div>
                            <div v-if="project.deal">
                                <div class="text-xs uppercase tracking-wide text-slate-400">{{ $e('Из сделки') }}</div>
                                <Link v-if="canOpenDeal" :href="route('deals.show', project.deal.id)" class="mt-0.5 block font-medium text-indigo-600 dark:text-indigo-400 hover:underline">{{ project.deal.number }} →</Link>
                                <div v-else class="mt-0.5 font-medium text-slate-900 dark:text-slate-100">{{ project.deal.number }}</div>
                            </div>
                        </div>

                        <div v-if="deal?.note" class="mt-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 px-4 py-2.5 ring-1 ring-inset ring-amber-100">
                            <div class="mb-0.5 text-xs font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-400">{{ $e('Заметка менеджера') }}</div>
                            <p class="whitespace-pre-line text-slate-700 dark:text-slate-300">{{ deal.note }}</p>
                        </div>
                        <div v-if="project.description || deal?.description" class="mt-4">
                            ...
                            <p class="mt-0.5 whitespace-pre-line text-slate-700 dark:text-slate-300">{{ project.description || deal?.description }}</p>
                        </div>
                    </div>
            ...
            <div v-if="canSeeMoney && finance" class="rounded-2xl bg-white dark:bg-slate-900/70 p-5 border border-slate-100 dark:border-slate-800 shadow-sm self-start">
                <div class="text-xs uppercase text-slate-400">{{ $e('Бюджет (сумма)') }}</div>
                <div class="mt-1 text-2xl font-bold text-indigo-600 dark:text-indigo-400">{{ money(finance.budget) }}</div>
                <div class="mt-4 space-y-2 text-sm">
                    <div class="flex justify-between"><span class="text-slate-500 dark:text-slate-400">{{ $e('Статус') }}</span><StatusBadge :status="project.status" /></div>
                    <div class="flex justify-between"><span class="text-slate-500 dark:text-slate-400">{{ $e('Расходы') }}</span><span class="font-medium text-red-600 dark:text-rose-400">{{ money(finance.expense) }}</span></div>
                    <div class="flex justify-between"><span class="text-slate-500 dark:text-slate-400">{{ $e('Прибыль') }}</span><span class="font-medium" :class="finance.plannedProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-rose-400'">{{ money(finance.plannedProfit) }}</span></div>
                    <div class="flex justify-between border-t pt-2"><span class="text-slate-500 dark:text-slate-400">{{ $e('Маржа') }}</span><span class="font-bold">{{ finance.plannedMargin }}% · {{ money(finance.plannedProfit) }}</span></div>
                </div>
            </div>
            <div v-else class="rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 p-4 text-sm text-indigo-700 dark:text-indigo-300 self-start">
