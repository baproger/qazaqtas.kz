            ...
            <div class="rounded-2xl border border-slate-100 bg-white p-8 text-center shadow-sm">
                ...
                <form @submit.prevent="submit" class="mt-6">
                    ...
                    <button :disabled="form.processing || !form.code"
                        class="mt-4 w-full rounded-2xl bg-indigo-600 py-3.5 text-base font-semibold text-white transition hover:bg-indigo-700 disabled:opacity-40">{{ $e('Открыть экран') }}</button>
                </form>
            </div>
            <p class="mt-4 text-center text-xs text-slate-400">{{ $e('QAZAQ TAS ERP · экран обновляется автоматически') }}</p>
        </div>
    </div>
</template>
