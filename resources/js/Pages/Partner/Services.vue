...
<template>
    ...
    <AppLayout>
        ...
        <div class="grid gap-3">
            <div v-for="s in services" :key="s.id" class="flex flex-wrap items-center gap-4 rounded-2xl border border-white/60 bg-white/75 p-4 shadow-soft backdrop-blur-md">
                <img v-if="s.thumb" :src="s.thumb" class="h-16 w-24 rounded-xl object-cover" :alt="s.title" />
                <div v-else class="flex h-16 w-24 items-center justify-center rounded-xl bg-slate-100 text-slate-300">◆</div>
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <a v-if="s.public_url" :href="s.public_url" target="_blank" class="text-xs font-medium text-indigo-600 hover:underline">{{ $e('на сайте') }} →</a>
                    </div>
                    <div class="mt-0.5 text-xs text-slate-500">{{ s.category?.name }}<template v-if="s.price"> · {{ new Intl.NumberFormat('ru-RU').format(s.price) }} ₸</template></div>
                    <div v-if="s.status === 'rejected' && s.rejection_reason" class="mt-1 rounded-lg bg-rose-50 px-2 py-1 text-xs text-rose-700">{{ $e('Причина:') }} {{ s.rejection_reason }}</div>
                </div>
                <div class="flex gap-1 text-xs">
                    <button class="rounded-lg px-2.5 py-1 font-medium text-slate-600 hover:bg-indigo-50 hover:text-indigo-700" @click="open(s)">{{ $e('Изменить') }}</button>
                    <button class="rounded-lg px-2.5 py-1 font-medium text-slate-400 hover:bg-rose-50 hover:text-rose-600" @click="remove(s)">{{ $e('Удалить') }}</button>
                </div>
            </div>
            <div v-if="!services.length" class="rounded-3xl border border-dashed border-slate-200 bg-white/60 p-12 text-center text-sm text-slate-400 backdrop-blur">◆ {{ $e('Добавьте первую услугу — после проверки она появится на сайте.') }}</div>
        </div>
        ...
        <div v-if="editing" class="fixed inset-0 z-40 flex items-start justify-center overflow-y-auto bg-slate-900/30 p-4 backdrop-blur-sm sm:p-8" @click.self="editing = null">
            <div class="w-full max-w-2xl rounded-3xl border border-white/60 bg-gradient-to-br from-white/95 via-indigo-50/70 to-violet-50/60 p-6 shadow-soft-lg backdrop-blur-xl">
                <h2 class="text-lg font-semibold text-slate-900">{{ editing === 'new' ? $e('Новая услуга') : $e('Изменить услугу') }}</h2>
                <p class="mt-0.5 text-xs text-slate-500">{{ $e('После сохранения услуга уйдёт на проверку (до 24 часов).') }}</p>
                <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-xs font-medium text-slate-500">{{ $e('Название услуги') }} *</label>
                    ...
                    <div>
                        ...
                        <div v-if="form.errors.category_id" class="mt-1 text-xs text-rose-600">{{ form.errors.category_id }}</div>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500">{{ $e('Цена, ₸ (пусто — договорная)') }}</label>
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500">{{ $e('Контактное лицо') }} *</label>
                        <input v-model="form.contact_name" type="text" :class="field" />
                        <div v-if="form.errors.contact_name" class="mt-1 text-xs text-rose-600">{{ form.errors.contact_name }}</div>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500">{{ $e('Телефон') }} *</label>
                        <input v-model="form.contact_phone" type="tel" :class="field" placeholder="+7 (___) ___-__-__" />
                        <div v-if="form.errors.contact_phone" class="mt-1 text-xs text-rose-600">{{ form.errors.contact_phone }}</div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div v-if="form.errors.photo" class="mt-1 text-xs text-rose-600">{{ form.errors.photo }}</div>
                    </div>
                </div>
