...
<template>
    ...
    <FinanceLayout :title="$e('Счета')" :subtitle="$e('выставлено, оплачено, остаток к оплате')" width="max-w-7xl">
            ...
            <div class="overflow-hidden rounded-2xl bg-white dark:bg-slate-900/70 shadow-sm border border-slate-200 dark:border-slate-800/80">
                <div class="flex flex-wrap items-center justify-between gap-2 border-b px-6 py-4">
                    <span class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $e('Счета') }}</span>
                    <!-- Дебиторка: выставлено / оплачено / клиенты должны -->
                    <div class="flex flex-wrap gap-2 text-xs">
                        <span class="rounded-full bg-slate-100 dark:bg-slate-800/60 px-2.5 py-1 font-medium text-slate-600 dark:text-slate-300">{{ $e('выставлено') }} <b class="tabular-nums whitespace-nowrap">{{ money(invoiceTotals.invoiced) }}</b></span>
                        <span class="rounded-full bg-emerald-100 dark:bg-emerald-500/20 px-2.5 py-1 font-medium text-emerald-700 dark:text-emerald-400">{{ $e('оплачено') }} <b class="tabular-nums whitespace-nowrap">{{ money(invoiceTotals.paid) }}</b></span>
                        <span class="rounded-full px-2.5 py-1 font-medium" :class="invoiceTotals.debt > 0 ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-400' : 'bg-slate-100 dark:bg-slate-800/60 text-slate-400'">{{ $e('долг клиентов') }} <b class="tabular-nums whitespace-nowrap">{{ money(invoiceTotals.debt) }}</b></span>
                    </div>
                </div>
                <!-- Сегодняшние счета -->
                <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        <tr v-for="inv in invoicesToday" :key="inv.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/60">
                            ...
                            <td class="px-4 py-3">
                                <Link v-if="inv.link && inv.link.id" :href="route(inv.link.type === 'project' ? 'projects.show' : 'deals.show', inv.link.id)"
                                    class="text-indigo-600 dark:text-indigo-400 hover:underline">{{ inv.link.label }}</Link>
                                <span v-else-if="inv.link" class="text-slate-400">{{ inv.link.label }}</span>
                                <span v-else class="text-slate-400">—</span>
                            </td>
                            <td class="px-4 py-3 text-slate-500 dark:text-slate-400">{{ inv.client?.name ?? '—' }}</td>
                            <td class="px-4 py-3">{{ money(inv.amount) }}</td>
                            <td class="px-4 py-3 text-green-600 dark:text-green-400">{{ money(inv.payments_sum_amount ?? 0) }}</td>
                ...
                <div class="border-t border-slate-100 dark:border-slate-800">
                    <button type="button" @click="invPastOpen = !invPastOpen" class="flex w-full items-center justify-between gap-3 px-6 py-3.5 text-left">
                        <div class="flex min-w-0 items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $e('Прошлые счета') }}</span>
                        </div>
                        <span class="flex-shrink-0 rounded-full bg-slate-100 dark:bg-slate-800/60 px-2.5 py-1 text-xs font-bold tabular-nums whitespace-nowrap text-slate-600 dark:text-slate-300">{{ invoicesPastStats?.count ?? 0 }} · {{ money(invoicesPastStats?.sum) }}</span>
                    </button>
                    <div v-show="invPastOpen" class="border-t border-slate-100 dark:border-slate-800">
                        ...
                        <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                            <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                                <tr v-for="inv in invoicesPast" :key="inv.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/60">
                                    <td class="px-4 py-3">
