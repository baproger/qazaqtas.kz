    ...
    <FinanceLayout :title="$e('Расходы')" :subtitle="$e('очередь на проверку и оплаченные за месяц')" width="max-w-7xl">
            ...
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-3">
                    <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $e('Требуют проверки') }}</h3>
                    <span v-if="pending.length" class="rounded-full bg-amber-100 dark:bg-amber-500/20 px-2.5 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-400">
                        {{ pending.length }} · <b class="tabular-nums whitespace-nowrap">{{ money(pendingTotal) }}</b>
                    </span>
                </div>
                <div v-if="canConfirm" class="flex items-center gap-2">
                    <button @click="showCats = true"
                        class="rounded-lg border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/70 px-3 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-slate-800/60">{{ $e('⚙ Категории') }}</button>
            ...
            <div v-if="!pending.length" class="rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/70 px-6 py-8 text-center text-sm text-slate-400 shadow-sm">
                {{ $e('Очередь пуста ✓') }}
            </div>

            <TransitionGroup v-else name="card" tag="div" class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                <div v-for="e in pending" :key="e.id" class="rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-4 shadow-sm">
                    <div class="flex flex-wrap items-start justify-between gap-2">
                        <div class="min-w-0">
                            <div class="text-xl font-bold tabular-nums whitespace-nowrap text-slate-900 dark:text-slate-100">{{ money(e.amount) }}</div>
                            <div class="mt-0.5 flex flex-wrap items-center gap-1.5 text-xs text-slate-400">
                                <span>{{ formatDate(e.date) }}</span>
                                <span v-if="e.category" class="rounded-full bg-slate-100 dark:bg-slate-800/60 px-2.5 py-0.5 font-medium text-slate-500 dark:text-slate-400">{{ e.category }}</span>
                                <Link v-if="e.source" :href="sourceUrl(e.source)" class="font-medium text-indigo-600 dark:text-indigo-400 hover:underline">
                                    {{ e.source.number || $e('без номера') }}<template v-if="e.source.title"> · {{ e.source.title }}</template>
                                </Link>
                                <span v-else class="rounded-full bg-indigo-50 dark:bg-indigo-500/10 px-2.5 py-0.5 font-medium text-indigo-700 dark:text-indigo-300">{{ $e('Расход компании') }}</span>
                            </div>
                        </div>
                        <div class="text-right text-xs text-slate-400">
                            {{ $e('подал') }}
                            <Link v-if="e.author" :href="route('users.show', e.author.id)" class="font-medium text-slate-600 dark:text-slate-300 hover:text-indigo-600">{{ e.author.name }}</Link>
                            <span v-else>—</span>
                        </div>
                    </div>

                    <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">{{ e.description || $e('без описания') }}</p>
                    ...
                    <a v-if="e.receipt?.kind === 'image'" :href="receiptUrl(e.id)" target="_blank" class="mt-2 block">
                        <img :src="receiptUrl(e.id)" :alt="$e('Чек')" class="max-h-44 w-full rounded-lg border border-slate-100 dark:border-slate-800 object-contain" />
                    </a>

                    <div class="mt-3 flex items-center justify-between gap-2">
                        <a v-if="e.receipt?.kind === 'pdf'" :href="receiptUrl(e.id)" target="_blank"
                            class="text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:underline">📄 {{ $e('Чек PDF') }}</a>
                        <span v-else-if="!e.receipt" class="rounded-full bg-amber-50 dark:bg-amber-500/10 px-2 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-400">{{ $e('без чека') }}</span>
                        <span v-else></span>
            ...
            <!-- ================= Оплаченные за месяц ================= -->
            <div class="mt-8 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/70 shadow-sm">
                <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 px-6 py-4">
                    <div class="flex items-center gap-3">
                        <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $e('Оплаченные за месяц') }}</h3>
                        <span class="rounded-full bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-700 dark:text-emerald-400 tabular-nums whitespace-nowrap">{{ money(paidTotal) }}</span>
                    </div>
                    <div class="flex flex-wrap items-center gap-2">
                        <button v-for="f in [{ k: 'kind', v: 'material', l: $e('материалы') }, { k: 'kind', v: 'other', l: $e('прочие') }, { k: 'method', v: 'cash', l: $e('наличные') }, { k: 'method', v: 'bank', l: $e('банк') }]"
                            ...
                            :class="(f.k === 'kind' ? kind : method) === f.v ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300' : 'border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/70 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/60'">{{ f.l }}</button>
                ...
                <div v-else class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                            <tr v-for="e in paid.data" :key="e.id" class="group transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-slate-800/60/60">
                                <td class="px-6 py-2.5 text-slate-500 dark:text-slate-400">{{ formatDate(e.date) }}</td>
                                <td class="px-4 py-2.5 text-right font-semibold tabular-nums whitespace-nowrap text-slate-800 dark:text-slate-200">{{ money(e.amount) }}</td>
                                <td class="px-4 py-2.5 text-slate-500 dark:text-slate-400">
                                    {{ e.category || '—' }}
                                    <span v-if="e.payout" class="ml-1 rounded-full px-2 py-0.5 text-xs font-medium"
                                        :class="e.payout === 'debt' ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400' : 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300'">{{ payoutLabel(e.payout) }}</span>
                                </td>
                                <td class="px-4 py-2.5 text-slate-500 dark:text-slate-400">{{ e.description || '—' }}</td>
                                ...
                                <td class="px-4 py-2.5 text-slate-500 dark:text-slate-400">{{ e.author?.name || '—' }}</td>
                                <td class="px-4 py-2.5">
                                    <span class="rounded-full bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-700 dark:text-emerald-400">{{ methodLabel(e.payment_method) }}</span>
                                    <a v-if="e.receipt" :href="receiptUrl(e.id)" target="_blank"
                                        class="ml-2 text-xs font-medium text-indigo-600 dark:text-indigo-400 opacity-0 transition-opacity hover:underline group-hover:opacity-100">{{ $e('чек') }}</a>
                                </td>
                                <td v-if="canConfirm" class="whitespace-nowrap px-4 py-2.5 text-right">
                                    <button class="rounded p-1 text-slate-300 dark:text-slate-600 transition-colors hover:text-indigo-600" :title="$e('Редактировать расход')" @click="openEdit(e)">
                                    ...
                                    <button class="rounded p-1 text-slate-300 dark:text-slate-600 transition-colors hover:text-rose-600" :title="$e('Удалить расход')" @click="removeExpense(e)">
                ...
                <div v-if="paid.data.length" class="border-t border-slate-100 dark:border-slate-800 p-4"><Pagination :links="paid.links" /></div>
            </div>

        <!-- ================= Подтверждение оплаты ================= -->
        <Modal :show="!!confirming" @close="confirming = null" max-width="lg">
            <div v-if="confirming" class="p-6">
                <h2 class="mb-1 text-lg font-semibold text-slate-900 dark:text-slate-100">{{ $e('Проверил, оплатить') }}</h2>
                <p class="mb-4 text-xs text-slate-400">{{ $e('Откуда ушли деньги — из кассы или с банковского счёта. Остатки пересчитаются сразу.') }}</p>
                <div class="mb-4 rounded-lg bg-slate-50 dark:bg-slate-800/50 px-4 py-3 text-sm">
                    <span class="font-semibold tabular-nums whitespace-nowrap text-slate-800 dark:text-slate-200">{{ money(confirming.amount) }}</span>
                    <span class="text-slate-500 dark:text-slate-400"> · {{ confirming.category || $e('без категории') }} · {{ confirming.description || $e('без описания') }}</span>
                </div>
                ...
                <div v-if="form.errors.payment_method" class="mt-1 text-xs text-red-600 dark:text-rose-400">{{ form.errors.payment_method }}</div>

                <div class="mt-4">
                    <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ confirming.receipt ? $e('Заменить чек (необязательно)') : $e('Чек (фото или PDF) *') }}</label>
        ...
        <Modal :show="!!editing" @close="editing = null" max-width="lg">
            <div v-if="editing" class="p-6">
                <h2 class="mb-4 text-lg font-semibold text-slate-900 dark:text-slate-100">{{ $e('Редактировать расход') }}</h2>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <p v-if="editing.material" class="mt-1 text-xs text-slate-400">{{ $e('Сумма списания считается автоматически (кол-во × цена).') }}</p>
                        <div v-if="eForm.errors.amount" class="mt-1 text-xs text-red-600 dark:text-rose-400">{{ eForm.errors.amount }}</div>
                    </div>
                    <div>
                    ...
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $e('За что') }}</label>
