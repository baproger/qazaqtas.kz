...
const statusStyle = { queued: 'bg-sky-50 text-sky-700', waiting: 'bg-slate-100 text-slate-600', running: 'bg-amber-50 text-amber-700', done: 'bg-emerald-50 text-emerald-700', skipped: 'bg-slate-100 text-slate-500', failed: 'bg-rose-50 text-rose-700' };
    ...
    <SettingsLayout :title="$e('Автоматизация')" wide>
        <div class="mb-4 flex flex-wrap items-center gap-2">
            <div class="flex rounded-lg border border-slate-200 bg-white p-0.5 text-sm">
                <button type="button" @click="tab = 'robots'" class="rounded-md px-3 py-1.5 font-medium" :class="tab === 'robots' ? 'bg-slate-900 text-white' : 'text-slate-600'">{{ $e('Роботы') }} <span class="opacity-60">{{ robots.length }}</span></button>
                <button type="button" @click="tab = 'runs'" class="rounded-md px-3 py-1.5 font-medium" :class="tab === 'runs' ? 'bg-slate-900 text-white' : 'text-slate-600'">{{ $e('Журнал') }} <span class="opacity-60">{{ runs.length }}</span></button>
            </div>
        ...
        <div v-if="tab === 'robots'" class="space-y-4">
            <div v-if="!byStage.length" class="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center">
                <div class="text-3xl">🤖</div>
                <div class="mt-2 text-sm font-medium text-slate-700">{{ $e('Роботов пока нет') }}</div>
            ...
            <section v-for="g in byStage" :key="g.stage.id ?? 'any'" class="rounded-2xl border border-slate-100 bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-slate-100 px-5 py-3">
                    <div class="flex items-center gap-2 text-sm font-semibold text-slate-800"><span class="h-2.5 w-2.5 rounded-full" :style="{ backgroundColor: g.stage.color }" />{{ g.stage.name }}</div>
                    <button type="button" @click="open(null, g.stage.id)" class="text-xs font-medium text-indigo-600 hover:underline">+ {{ $e('добавить') }}</button>
                </div>
                <div class="divide-y divide-slate-50">
                    <div v-for="r in g.robots" :key="r.id" class="flex flex-wrap items-center gap-3 px-5 py-3" :class="r.is_active ? '' : 'opacity-50'">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="mt-0.5 flex flex-wrap gap-x-2 text-xs text-slate-500">
                                <span class="rounded-full bg-slate-100 px-2 py-0.5">{{ triggers[r.trigger] }}</span>
                                <span class="rounded-full bg-slate-100 px-2 py-0.5">{{ delayLabel(r.delay_seconds) }}</span>
                                <span v-if="r.sequence === 'sequential'" class="rounded-full bg-slate-100 px-2 py-0.5">{{ $e('по цепочке') }}</span>
                                <span v-if="conditionText(r)" class="rounded-full bg-amber-50 px-2 py-0.5 text-amber-700">{{ $e('если') }} {{ conditionText(r) }}</span>
                                <span class="rounded-full bg-indigo-50 px-2 py-0.5 font-medium text-indigo-700">{{ actionLabel(r.action_type) }}</span>
                                <span v-if="r.failed_runs_count" class="rounded-full bg-rose-50 px-2 py-0.5 text-rose-700">{{ $e('ошибок:') }} {{ r.failed_runs_count }}</span>
                            </div>
                        </div>
                        <div class="flex gap-1 text-xs">
                            <button class="rounded-lg px-2 py-1 font-medium text-slate-600 hover:bg-indigo-50 hover:text-indigo-700" @click="open(r)">{{ $e('Изменить') }}</button>
                            <button class="rounded-lg px-2 py-1 font-medium text-slate-500 hover:bg-slate-100" @click="duplicate(r)">{{ $e('Копия') }}</button>
                    ...
                    <div v-if="!g.robots.length" class="px-5 py-3 text-xs text-slate-300">{{ $e('Роботов нет') }}</div>
                </div>
            </section>
        </div>

        <!-- ===== Журнал ===== -->
        <div v-else class="overflow-x-auto rounded-2xl border border-slate-100 bg-white shadow-sm">
            <table class="min-w-full divide-y divide-slate-100 text-sm">
                ...
                <tbody class="divide-y divide-slate-50">
                    <tr v-for="r in runs" :key="r.id">
                        ...
                        <td class="px-4 py-2"><div class="font-medium text-slate-800">{{ r.robot }}</div><div class="text-xs text-slate-400">{{ actionLabel(r.action) }}</div></td>
                        <td class="px-4 py-2"><a v-if="r.deal" :href="route('deals.show', r.deal.id)" class="font-semibold text-indigo-600 hover:underline">{{ r.deal.number }}</a><div class="text-xs text-slate-400">{{ r.deal?.company }}</div></td>
                        ...
                        <td class="max-w-md px-4 py-2 text-xs text-slate-500"><span v-if="r.error" class="text-rose-600">{{ r.error }}</span><span v-else-if="r.output">{{ JSON.stringify(r.output) }}</span><span v-else>—</span></td>
                    </tr>
                    <tr v-if="!runs.length"><td colspan="5" class="px-4 py-8 text-center text-xs text-slate-300">{{ $e('Запусков ещё не было') }}</td></tr>
                </tbody>
            </table>
        </div>
        ...
        <div v-if="editing" class="fixed inset-0 z-40 flex items-start justify-center overflow-y-auto bg-slate-900/30 p-4 backdrop-blur-sm sm:p-8" @click.self="editing = null">
            <div class="w-full max-w-3xl rounded-3xl border border-white/60 bg-gradient-to-br from-white/95 via-indigo-50/70 to-violet-50/60 p-6 shadow-soft-lg backdrop-blur-xl">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-slate-900">{{ editing === 'new' ? $e('Новый робот') : $e('Робот') }}</h2>
                    <label class="flex items-center gap-2 text-sm text-slate-600"><Toggle v-model="form.is_active" /> {{ $e('Включён') }}</label>
                </div>
                ...
                <div class="mt-4 rounded-2xl bg-white/60 p-4">
                    ...
                    <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <div>
                            <label class="mb-1 block text-xs text-slate-500">{{ $e('Этап') }}</label>
                        ...
                        <div>
                            <label class="mb-1 block text-xs text-slate-500">{{ $e('Событие') }}</label>
                            <select v-model="form.trigger" :class="fieldClass"><option v-for="(l, k) in triggers" :key="k" :value="k">{{ l }}</option></select>
                        </div>
                        <div>
                            <label class="mb-1 block text-xs text-slate-500">{{ $e('Задержка') }}</label>
                        ...
                        <div>
                            <label class="mb-1 block text-xs text-slate-500">{{ $e('Очерёдность') }}</label>
                            <select v-model="form.sequence" :class="fieldClass"><option v-for="(l, k) in sequences" :key="k" :value="k">{{ l }}</option></select>
                            <label class="mt-2 flex items-center gap-2 text-xs text-slate-600"><input type="checkbox" v-model="form.run_if_left" class="rounded border-slate-300 text-indigo-600" /> {{ $e('Выполнить, даже если сделка уже ушла с этапа') }}</label>
                        </div>
                    </div>
                </div>

                <!-- ЕСЛИ -->
                <div class="mt-3 rounded-2xl bg-white/60 p-4">
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button" @click="addCondition" class="text-xs font-medium text-indigo-600 hover:underline">+ {{ $e('условие') }}</button>
                    </div>
                ...
                <div class="mt-3 rounded-2xl bg-white/60 p-4">
                    ...
                    <div class="mt-2 flex flex-wrap gap-2">
                        <button v-for="a in actions" :key="a.type" type="button" @click="form.action_type = a.type; form.action_payload = {}"
                            ...
                            :class="form.action_type === a.type ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-white/70 bg-white/70 text-slate-600 hover:bg-white'">{{ a.label }}</button>
                    </div>
                    <div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <div v-for="f in schemaOf(form.action_type)" :key="f.key" :class="['textarea'].includes(f.type) ? 'sm:col-span-2' : ''">
                            <label class="mb-1 block text-xs text-slate-500">{{ f.label }}<span v-if="f.required" class="text-rose-500"> *</span></label>
                            ...
                            <label v-else-if="f.type === 'bool'" class="flex h-10 items-center gap-2 text-sm text-slate-700"><Toggle :model-value="!!form.action_payload[f.key]" @update:model-value="(v) => (form.action_payload[f.key] = v)" /> {{ $e('да') }}</label>
