...
const levels = {
    critical: { label: tr('Критические'), one: tr('Критическая'), dot: 'bg-rose-600', badge: 'bg-rose-50 text-rose-700 ring-rose-200', tile: 'from-rose-50 to-white border-rose-100 text-rose-700' },
    error: { label: tr('Ошибки'), one: tr('Ошибка'), dot: 'bg-orange-500', badge: 'bg-orange-50 text-orange-700 ring-orange-200', tile: 'from-orange-50 to-white border-orange-100 text-orange-700' },
    warning: { label: tr('Предупреждения'), one: tr('Предупреждение'), dot: 'bg-amber-400', badge: 'bg-amber-50 text-amber-700 ring-amber-200', tile: 'from-amber-50 to-white border-amber-100 text-amber-700' },
    info: { label: tr('Простые'), one: tr('Простая'), dot: 'bg-sky-400', badge: 'bg-sky-50 text-sky-700 ring-sky-200', tile: 'from-sky-50 to-white border-sky-100 text-sky-700' },
};
    ...
    <AppLayout>
        ...
        <div class="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
            <button v-for="(l, key) in levels" :key="key" type="button" @click="setLevel(key)"
                ...
                <div class="mt-2 text-3xl font-bold tabular-nums text-slate-900">{{ stats[key]?.n ?? 0 }}</div>
                <div class="mt-0.5 text-xs text-slate-500">{{ $e('за сутки') }} · {{ stats[key]?.hits ?? 0 }} {{ $e('повторов') }}</div>
            </button>
        </div>
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2">
            <div class="flex rounded-lg border border-slate-200 bg-white p-0.5 text-sm">
                <button v-for="s in [['open', tr('Открытые')], ['resolved', tr('Разобранные')], ['all', tr('Все')]]" :key="s[0]" type="button"
                    ...
                    :class="fStatus === s[0] ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'">{{ s[1] }}</button>
            </div>
            ...
            <button v-if="openTotal" type="button" @click="resolveAll"
                class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">{{ $e('Разобрать все') }}</button>
            <button type="button" @click="purge"
                class="rounded-lg px-3 py-1.5 text-xs font-medium text-slate-400 hover:bg-slate-100 hover:text-rose-600">{{ $e('Очистить старые') }}</button>
        </div>

        <!-- Список -->
        <div v-if="logs.data.length" class="divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
            <div v-for="e in logs.data" :key="e.id" :class="e.resolved_at ? 'opacity-60' : ''">
                <button type="button" @click="toggle(e.id)" class="flex w-full items-start gap-3 px-5 py-3.5 text-left transition hover:bg-slate-50">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span v-if="e.resolved_at" class="rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200">{{ $e('Разобрано') }}</span>
                        </div>
                        <div class="mt-1 truncate text-sm font-medium text-slate-900">{{ e.message }}</div>
                    ...
                    <div class="shrink-0 text-right text-xs text-slate-400">
                        <div class="font-medium text-slate-600">{{ ago(e.last_seen_at) }}</div>
                        <div>{{ fmt(e.last_seen_at) }}</div>
                    </div>
                </button>

                <div v-if="open.has(e.id)" class="border-t border-slate-100 bg-slate-50/70 px-5 py-4 text-sm">
                    <dl class="grid gap-x-6 gap-y-2 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div v-if="e.user_agent" class="lg:col-span-3"><dt class="text-xs uppercase text-slate-400">{{ $e('Браузер') }}</dt><dd class="truncate text-slate-500">{{ e.user_agent }}</dd></div>
                    ...
                    <div class="mt-3 flex justify-end">
                        <button type="button" @click="resolve(e)"
                            ...
                            :class="e.resolved_at ? 'border border-slate-200 bg-white text-slate-600 hover:bg-slate-50' : 'bg-emerald-600 text-white hover:bg-emerald-700'">
        ...
        <div v-else class="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center">
            ...
            <div class="mt-2 text-sm font-medium text-slate-700">{{ $e('Ошибок нет') }}</div>
