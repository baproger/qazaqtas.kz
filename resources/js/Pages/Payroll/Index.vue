                ...
                <table class="min-w-full divide-y divide-slate-100 text-xs">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <tr v-for="d in me.dealsList" :key="d.id" class="hover:bg-slate-50">
                            ...
                            <td class="px-3 py-2"><span :class="d.is_won ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'" class="rounded-full px-2 py-0.5 text-xs font-medium">{{ d.stage }}</span></td>
                            ...
                            <td class="px-3 py-2 text-right font-semibold tabular-nums text-emerald-600">
                                {{ money(d.bonus) }}
                                <span v-if="d.bonus_manual" class="ml-1 rounded bg-amber-100 px-1 py-px text-xs font-bold uppercase text-amber-700" :title="$e('Ручной % финансиста: ') + d.bonus_rate + '%'">{{ d.bonus_rate }}%</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <template v-else>
            ...
            <div class="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
                ...
                <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
                    <div class="truncate text-xs uppercase tracking-wide text-emerald-600/70">{{ $e('К выплате ·') }} {{ monthLabel }}</div>
                                ...
                                <div class="flex items-center justify-between gap-2">
                                    <span class="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-slate-600">
                                        ...
                                        <span v-if="g.id != null" class="normal-case tracking-normal" @click.stop>
                                            ...
                                            <button v-else-if="canManage" class="group/norm inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold tabular-nums transition-colors"
                                            ...
                                            <span v-else class="rounded-full px-2 py-0.5 text-xs font-semibold tabular-nums" :class="g.override ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-200/70 text-slate-500'">{{ $e('норма') }} {{ g.norm }} {{ $e('ч') }}</span>
                                        </span>
                                    </span>
                        ...
                        <template v-for="r in g.list" :key="r.uid">
                            <tr class="cursor-pointer hover:bg-slate-50" @click="toggle(r.uid)">
                                ...
                                <td class="px-4 py-3 text-right tabular-nums" :class="r.bonus > 0 ? 'font-medium text-emerald-600' : 'text-slate-300'">
                                    ...
                                    <div v-if="r.bonus_left > 0 && r.bonus_left !== r.bonus" class="text-xs text-amber-600">{{ $e('не выплачено') }} {{ money(r.bonus_left) }}</div>
                                </td>
                                <td class="px-4 py-3 text-right tabular-nums" :class="r.deductions > 0 ? 'text-rose-600 font-medium' : 'text-slate-300'">
                                    ...
                                    <div v-if="r.debt_charge > 0" class="text-xs font-medium text-rose-500">{{ $e('долг') }} − {{ money(r.debt_charge) }}</div>
                                </td>
                                ...
                                <td colspan="6" class="px-4 py-3">
                                    ...
                                    <div class="mb-3 rounded-lg border border-slate-200 bg-white p-3">
                                        <div class="mb-1 flex items-center justify-between">
                                            <span class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $e('Корректировки ·') }} {{ monthLabel }}</span>
                                    ...
                                    <div class="mb-3 rounded-lg border border-slate-200 bg-white p-3">
                                        <div class="mb-1 flex items-center justify-between">
                                            <span class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $e('Долг') }}</span>
                                        ...
                                        <template v-if="r.debt && r.debt.items.length">
                                            ...
                                            <div class="divide-y divide-slate-50 text-xs">
                                                <div v-for="d in r.debt.items" :key="d.id" class="flex items-center justify-between gap-2 py-1.5">
                                                    <div class="flex items-center gap-2">
                                                        <span class="rounded-full bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-700">{{ $e('Долг') }}</span>
                                        ...
                                        <table class="min-w-full divide-y divide-slate-100 text-xs">
                                            ...
                                            <tbody class="divide-y divide-slate-50">
                                                <tr v-for="d in r.dealsList" :key="d.id" class="hover:bg-slate-50">
                                                    ...
                                                    <td class="px-3 py-2">
                                                        <span :class="d.is_won ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'" class="rounded-full px-2 py-0.5 text-xs font-medium">{{ d.stage }}</span>
                                                    </td>
                                                    <td class="px-3 py-2 text-right tabular-nums text-slate-700">{{ money(d.budget) }}</td>
                                                    <td class="px-3 py-2 text-right tabular-nums" :class="d.paid >= d.budget ? 'text-emerald-600' : 'text-slate-500'">{{ money(d.paid) }}</td>
