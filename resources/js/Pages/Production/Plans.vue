    ...
    <AppLayout>
        ...
        <div class="mx-auto max-w-7xl">
            ...
            <div class="mb-5 flex flex-wrap items-end justify-between gap-3">
                <div>
                    <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">{{ $e('Задание цеху на месяц') }}</h2>
            ...
            <div v-if="!plans.length && !queue.length" class="mt-6 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800/80 px-6 py-16 text-center">
                <p class="text-sm text-slate-500 dark:text-slate-400">{{ isForeman ? $e('На этот месяц вам план не поставили.') : $e('Планов на этот месяц нет.') }}</p>
                <button v-if="canPlan" @click="openForm" class="mt-3 text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">{{ $e('Поставить первый план') }}</button>
            </div>
            ...
            <div v-else class="mt-6 space-y-6">
              ...
              <section v-if="queue.length">
                  <div class="mb-3 flex flex-wrap items-baseline gap-x-3">
                      <span class="text-sm font-semibold text-amber-800 dark:text-amber-300">{{ $e('Ждёт бригаду') }}</span>
                      <span class="rounded-full bg-amber-100 dark:bg-amber-500/20 px-2 py-0.5 text-xs font-semibold tabular-nums text-amber-800 dark:text-amber-300">{{ queue.length }}</span>
                      <span class="text-xs text-slate-400">{{ $e('пришло из сделок — назначьте, кто делает') }}</span>
                  </div>
                  <div class="overflow-hidden rounded-2xl border border-amber-200 bg-white dark:bg-slate-900/70 shadow-soft">
                      <div class="divide-y divide-amber-100/70">
                          <div v-for="row in queue" :key="row.id" class="flex flex-wrap items-center gap-x-4 gap-y-2 px-6 py-3.5">
                              <div class="min-w-0 flex-1">
                                  <div class="text-sm font-medium text-slate-900 dark:text-slate-100">🧱 {{ row.product }}</div>
                                  <div v-if="row.deal" class="text-xs text-slate-400">
                                      <Link :href="route('deals.show', row.deal.id)" class="font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">{{ row.deal.number }}</Link>
                                      · {{ row.deal.client }}
                                      <span v-if="row.deal.deadline"> · {{ $e('срок') }} {{ row.deal.deadline }}</span>
                                  </div>
                              </div>
                              <span class="text-sm font-semibold tabular-nums text-slate-900 dark:text-slate-100">{{ num(row.qty) }} {{ row.unit }}</span>
                              ...
                              <template v-if="canPlan">
                                  ...
                                  <button v-else @click="openAssign(row)"
                                      class="rounded-lg border border-amber-300 px-3 py-1.5 text-xs font-semibold text-amber-800 dark:text-amber-300 transition-colors duration-150 hover:bg-amber-50">
                                      {{ $e('Назначить бригаду') }}
                                  </button>
                              </template>
              ...
              <section v-for="section in sections" :key="section.key" v-show="section.groups.length">
                ...
                <button type="button" class="mb-3 flex w-full flex-wrap items-baseline gap-x-3 gap-y-1 text-left"
                    ...
                    <span class="flex items-center gap-1.5 text-sm font-semibold"
                        :class="section.tone === 'emerald' ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-900 dark:text-slate-100'">
                    ...
                    <span class="rounded-full px-2 py-0.5 text-xs font-semibold tabular-nums"
                        :class="section.tone === 'emerald' ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400' : 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300'">
                        {{ section.sum?.count ?? 0 }}
                    </span>
                    <span class="text-xs text-slate-400">{{ section.hint }}</span>
                    <span class="ml-auto flex flex-wrap items-baseline gap-x-3 text-xs tabular-nums text-slate-500 dark:text-slate-400">
                        <span v-for="m in section.sum?.measures ?? []" :key="m.measure">
                            {{ num(m.done) }} / {{ num(m.plan) }} {{ measureLabel(m.measure) }}
                        </span>
                        <b class="text-emerald-600 dark:text-emerald-400">{{ money(section.sum?.bonus ?? 0) }}</b>
                    </span>
                </button>

                <div v-show="sectionOpen(section)" class="space-y-4">
                <div v-for="b in section.groups" :key="section.key + '-' + b.id" class="overflow-hidden rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/70 shadow-soft">
                    ...
                    <Link :href="route('production.brigade', { brigade: b.id, month })"
                        class="flex flex-wrap items-center justify-between gap-x-6 gap-y-2 border-b border-slate-100 dark:border-slate-800 px-6 py-4 transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-slate-800/60">
                        <div class="min-w-0">
                            <span class="text-base font-semibold text-slate-900 dark:text-slate-100">👷 {{ b.name }}</span>
                        ...
                        <div class="flex flex-wrap items-baseline gap-x-4 gap-y-1 text-sm tabular-nums">
                            <span v-for="m in b.measures" :key="m.measure" class="text-slate-500 dark:text-slate-400">
                                <b :class="m.percent >= 100 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-slate-100'">{{ num(m.done) }}</b>
                                / {{ num(m.plan) }} <span class="text-xs">{{ measureLabel(m.measure) }}</span>
                                <span class="ml-1 text-xs" :class="m.percent >= 100 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'">{{ m.percent }}%</span>
                            </span>
                            <!-- Ждёт мастера: пока не подтвердил — это ещё не бонус. -->
                            <span v-if="b.pending" class="rounded bg-amber-50 dark:bg-amber-500/10 px-1.5 py-0.5 text-xs font-semibold text-amber-700 dark:text-amber-400"
                                :title="$e('внесено, но мастер ещё не подтвердил')">+{{ num(b.pending) }} {{ $e('ждёт') }}</span>
                            <b class="text-emerald-600 dark:text-emerald-400">{{ money(b.bonus) }}</b>
                            <span class="text-xs font-semibold text-indigo-600 dark:text-indigo-400">{{ isForeman ? $e('моя бригада') : $e('подробнее') }} →</span>
                        </div>
                    </Link>
                    ...
                    <div class="divide-y divide-slate-50 dark:divide-slate-800">
                        <div v-for="p in b.rows" :key="p.id" class="px-6 py-3.5">
                            <div class="flex flex-wrap items-baseline justify-between gap-3">
                                <span class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ p.product }}</span>
                                <div class="flex items-baseline gap-3 text-sm tabular-nums">
                                    <span><b :class="p.over ? 'text-amber-600 dark:text-amber-400' : 'text-slate-900 dark:text-slate-100'">{{ num(p.done) }}</b><span class="text-slate-400"> / {{ num(p.plan) }} {{ p.unit }}</span></span>
                                    <span class="w-12 text-right text-xs text-slate-400">{{ p.percent }}%</span>
                                    <span class="w-24 text-right text-emerald-600 dark:text-emerald-400">{{ money(p.bonus) }}</span>
                                    <button v-if="canPlan && p.editable" @click.stop="removePlan(p)" :title="$e('Убрать план')"
                                        class="rounded p-1 text-slate-300 dark:text-slate-600 transition-colors duration-150 hover:bg-rose-50 hover:text-rose-600">
                            ...
                            <div class="mt-1.5 h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800/60">
                                <div class="h-full rounded-full transition-all duration-500" :class="barClass(p)"
                                    :style="{ width: Math.min(p.percent, 100) + '%' }"></div>
                            </div>

                            <div class="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs tabular-nums text-slate-500 dark:text-slate-400">
                                <span v-if="p.left">{{ $e('осталось') }} <b class="text-slate-700 dark:text-slate-300">{{ num(p.left) }} {{ p.unit }}</b></span>
                                <span v-else class="font-semibold text-emerald-600 dark:text-emerald-400">{{ $e('план закрыт ✓') }}</span>
                                <span v-if="p.pending" class="text-amber-600 dark:text-amber-400">{{ $e('ждёт подтверждения') }} {{ num(p.pending) }}</span>
                            </div>

                            <!-- «Сделал» — только бригадиру: он вводит выработку,
                                 стоя у формы с телефоном, и лишний переход ему дорог. -->
                            <div v-if="isForeman" class="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 dark:border-slate-800 pt-3">
                                ...
                                <button v-else-if="p.status === 'active'" @click="openReport(p)"
                                    class="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 dark:bg-slate-800/60 px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 transition-colors duration-150 hover:bg-slate-200">
                            ...
                            <div v-if="isForeman && openRecords === p.id" class="mt-2 space-y-1 border-t border-slate-100 dark:border-slate-800 pt-2 text-xs">
                                <div v-for="o in ordersOf(p.id)" :key="o.id" class="flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
                                    ...
                                    <b class="tabular-nums text-slate-700 dark:text-slate-300">{{ num(o.qty) }} {{ o.unit }}</b>
                                    <span v-if="o.status === 'confirmed'" class="text-emerald-600 dark:text-emerald-400">✓ {{ $e('принято') }}</span>
                                    <span v-else-if="o.status === 'rejected'" class="text-rose-600 dark:text-rose-400">✕ {{ o.reject_reason }}</span>
                                    <span v-else class="text-amber-600 dark:text-amber-400">⏳ {{ $e('ждёт') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <section v-if="brigadeOutput.length">
                    <div class="mb-3 flex flex-wrap items-baseline gap-x-3">
                        <span class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $e('Выработка бригад за месяц') }}</span>
                        <span class="text-xs text-slate-400">{{ $e('только подтверждённые смены — неподтверждённое ещё не деньги') }}</span>
                    </div>
                    <div class="overflow-hidden rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/70 shadow-soft">
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-sm">
                                ...
                                <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                                    <tr v-for="b in brigadeOutput" :key="b.id" class="transition-colors duration-150 hover:bg-slate-50/60 dark:hover:bg-slate-800/40">
                                        <td class="px-6 py-2.5">
                                            <Link :href="route('production.brigade', { brigade: b.id, month })" class="font-medium text-slate-800 dark:text-slate-200 hover:text-indigo-600">👷 {{ b.name }}</Link>
                                            <span v-if="b.workshop" class="ml-1.5 text-xs text-slate-400">{{ b.workshop }}</span>
                                        </td>
                                        <td class="px-4 py-2.5 text-right tabular-nums text-slate-500 dark:text-slate-400">{{ b.shifts }}</td>
                                        <td class="px-4 py-2.5 text-right tabular-nums text-slate-600 dark:text-slate-300">{{ b.m2 ? num(b.m2) : '—' }}</td>
                                        <td class="px-4 py-2.5 text-right tabular-nums text-slate-600 dark:text-slate-300">{{ b.pcs ? num(b.pcs) : '—' }}</td>
                                        <td class="px-4 py-2.5 text-right tabular-nums text-slate-500 dark:text-slate-400">{{ money(b.plan_amount) }}</td>
                                        <td class="px-6 py-2.5 text-right font-semibold tabular-nums text-emerald-600 dark:text-emerald-400">{{ money(b.amount) }}</td>
                                    </tr>
                                </tbody>
                                <tfoot class="border-t border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40 text-xs">
                                    <tr>
                                        <td class="px-6 py-2.5 font-medium text-slate-500 dark:text-slate-400">{{ $e('Итого') }}</td>
                                        <td class="px-4 py-2.5 text-right tabular-nums text-slate-500 dark:text-slate-400">{{ outputTotals.shifts }}</td>
                                        <td class="px-4 py-2.5 text-right tabular-nums text-slate-600 dark:text-slate-300">{{ outputTotals.m2 ? num(outputTotals.m2) : '—' }}</td>
                                        <td class="px-4 py-2.5 text-right tabular-nums text-slate-600 dark:text-slate-300">{{ outputTotals.pcs ? num(outputTotals.pcs) : '—' }}</td>
                                        <td class="px-4 py-2.5 text-right tabular-nums text-slate-500 dark:text-slate-400">{{ money(outputTotals.plan_amount) }}</td>
                                        <td class="px-6 py-2.5 text-right font-semibold tabular-nums text-emerald-600 dark:text-emerald-400">{{ money(outputTotals.amount) }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
            ...
            <div v-if="!isForeman && orders.filter((o) => o.status === 'draft').length" class="mt-6 overflow-hidden rounded-2xl border border-amber-200 bg-white dark:bg-slate-900/70 shadow-soft">
                <div class="border-b border-amber-100 px-6 py-4 text-sm font-semibold text-slate-900 dark:text-slate-100">
                    ⏳ {{ $e('Ждут подтверждения') }}
                    <span class="ml-1 rounded-full bg-amber-50 dark:bg-amber-500/10 px-2 py-0.5 text-xs font-semibold text-amber-700 dark:text-amber-400">{{ summary.waiting }}</span>
                </div>
                <div class="divide-y divide-slate-50 dark:divide-slate-800">
                    <div v-for="o in orders.filter((x) => x.status === 'draft')" :key="o.id"
                        ...
                        <span class="tabular-nums text-slate-400">{{ o.date }}</span>
                        <span class="text-slate-700 dark:text-slate-300">{{ o.brigade }}</span>
                        <span class="text-slate-500 dark:text-slate-400">{{ o.product }}</span>
                        <b class="tabular-nums text-slate-900 dark:text-slate-100">{{ num(o.qty) }} {{ o.unit }}</b>
                        ...
                        <div v-if="canConfirm" class="ml-auto flex gap-2">
                            ...
                            <button @click="rejectOrder(o)"
                                class="rounded-lg border border-slate-200 dark:border-slate-800/80 px-3 py-1 text-xs font-semibold text-slate-500 dark:text-slate-400 transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-slate-800/60">{{ $e('Вернуть') }}</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <Modal :show="showForm" @close="showForm = false" max-width="lg">
            <div class="p-6">
                <h2 class="mb-1 text-lg font-semibold text-slate-900 dark:text-slate-100">{{ $e('План на месяц') }}</h2>
                <p class="mb-5 text-xs text-slate-400">{{ $e('Один план — один товар. Бригадир увидит его у себя сразу.') }}</p>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $e('Месяц') }}</label>
                        <input v-model="form.period_month" type="month" class="w-full rounded-lg border-slate-200 text-sm shadow-sm" />
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $e('Бригада') }}</label>
                    ...
                    <div class="col-span-2">
                        <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $e('Товар') }}</label>
                        <div v-if="pickedProduct" class="flex items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 dark:bg-indigo-500/10 px-3 py-2 text-sm">
                            <span class="font-medium text-indigo-900 dark:text-indigo-200">{{ pickedProduct.name }}</span>
                            <span class="text-xs text-indigo-500 dark:text-indigo-400">{{ pickedProduct.unit }}</span>
                            <button class="ml-auto text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:underline" @click="form.product_id = ''">{{ $e('Изменить') }}</button>
                        </div>
                        <div v-else class="max-h-60 overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-800/80">
                            <div v-for="c in productCategories" :key="c.id" class="border-b border-slate-100 dark:border-slate-800 last:border-0">
                                <button type="button" @click="openCategory = openCategory === c.id ? null : c.id"
                                    class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-slate-800/60">
                                    <svg class="h-3 w-3 shrink-0 text-slate-300 dark:text-slate-600 transition-transform duration-200" :class="openCategory === c.id ? 'rotate-90' : ''"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                                ...
                                <div v-if="openCategory === c.id" class="bg-slate-50/60 dark:bg-slate-800/40 pb-1">
                                    <button v-for="p in productsOf(c.id)" :key="p.id" type="button" @click="pickProduct(p)"
                                        class="flex w-full items-baseline gap-2 px-3 py-1.5 pl-8 text-left text-sm transition-colors duration-150 hover:bg-white dark:hover:bg-slate-800">
                        ...
                        <p v-if="form.errors.product_id" class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ form.errors.product_id }}</p>
                    </div>
                    <div>
                        ...
                        <p v-if="form.errors.plan_qty" class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ form.errors.plan_qty }}</p>
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ $e('Ставка бригадира, ₸ за единицу') }}</label>
